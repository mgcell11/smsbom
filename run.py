...
{kan}Tools Spam All In One""")
 ...
 def start():
  try:
      nom=str(input(f"{kun}[{kan}•{kun}] {im}Nomor (Contoh:85xx) : "))
      if len(nom) <= 5: print (f"{kun}[{kan}•{kun}] {kun}Masukan Nomor Sesuai Contoh");main.start()
      print (f"\r{kun}[{kan}•{kun}] {kan}Memulai spamming...")
...
'accept-language':'id-ID,id;q=0.9,en-US;q=0.8,en;q=0.7'
     ...
     dat = {"emailAddress":"abilseno11@gmail.com","firstName":"NUGELO","lastName":"Gile","mobileNumber":"0"+nom,"mccCardNumber":"","password":"savagetro123","referralCode":"","dateOfBirth":"03-01-1999","gender":"Male","registrationType":"N"}
     datj = json.dumps(dat)
     for x in range(5):
